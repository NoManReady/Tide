...
Verify.prototype.check = function (group, field) {
    var self = this;
    ...
    //遍历验证队列进行验证
    if (field) {
        verifyQueue = verifyQueue.filter(c => {
            return c === field
        })
    }
...
Verify.prototype.reset = function (errors) {
    var _errors = errors || this.vm.$verify.$errors;
    for (var key in _errors) {
        if (Array.isArray(_errors[key])) {
            _.set(_errors, key, [])
        } else {
            this.reset(_errors[key])
        }
    }
}
var init = function () {
...
var Directive = function (Vue, options) {
    Vue.directive("verify", {
        bind: function (el, binding, vnode, oldVnode) {
            ...
            vm.$watch(expression, function (val) {
                vm.$verify.check(null, expression)
            })
        }
    });
